...
class _HomePageState extends State<HomePage> {
  ...
  int _glassSize = 200; // ml per glass (now customizable)
  int _dailyGoal = 4000; // Default 4 liters (will be adjusted based on profile)
  ...
  void initState() {
    ...
    // Load saved water tracking data
  ...
  // Initialize water goal based on gender from profile
  void _initializeWaterGoal() {
    final profileProvider =
        Provider.of<ProfileProvider>(context, listen: false);

    // Default to 4000 ml (4 liters) if gender not specified
    int goalBasedOnGender = 4000;

    // Adjust based on gender if specified
    if (profileProvider.gender.toLowerCase() == 'male') {
      goalBasedOnGender = 5000; // 5 liters for men
    } else if (profileProvider.gender.toLowerCase() == 'female') {
      goalBasedOnGender = 4000; // 4 liters for women
    }

    // Get saved custom goal or use gender-based default
  ...
  Future<void> _loadWaterTrackingData() async {
    try {
      ...
      // Initialize water goal based on profile
  ...
  // Initialize water reminder timer - shows reminder every hour
  void _initWaterReminderTimer() {
    _lastWaterReminder = DateTime.now();

    // Update timer every minute to check if an hour has passed
    _waterReminderTimer = Timer.periodic(const Duration(minutes: 1), (timer) {
      ...
      // Show reminder if an hour has passed
      if (difference.inHours >= 1) {
        ...
        // Auto-hide the reminder after 15 seconds
  ...
  // Method to log a glass of water
  void _logWaterGlass() {
    if (_goalReached) return; // Safety check
  ...
  // Method to show dialog for custom water amount input
  ...
  // Method to log a custom water amount
  void _logCustomWaterAmount(int amountInMl) {
    if (amountInMl <= 0) return;

    setState(() {
      // Calculate equivalent glasses for our tracking
  ...
  // Method to customize glass size
  ...
  // Method to customize daily water goal
              ...
              if (goalController.text.isNotEmpty) {
                ...
                if (goal >= 1000) {
                  ...
                  setState(() {
                    ...
                    // Recalculate if goal is reached with new target
                    final int totalConsumed = _glassesConsumed * _glassSize;
  ...
  // Method to build a water glass icon
  ...
  // Format time until next water reminder
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          RefreshIndicator(
            ...
            child: ListView(
              ...
              children: [
                ...
                // Health Tip - placed after header
          ...
          // Floating water reminder notification
  ...
  String _getWellnessMessage() {
    ...
    final random = DateTime.now().microsecond % 8; // For more variety
